...
class FiberDiffractionIndexer:
    ...
    def __init__(
        ...
        self.png_output_dir = self.result_dir
    ...
    def _run_single_step(self, step: int, start_time: float) -> None:
        ...
        self.callback.on_progress(
            step, f"运行 Fortran 优化... 最佳误差: {best_error:.6f}"
        )
    ...
    def _generate_plots(self) -> None:
        ...
        使用 Plotter 生成默认 PNG，并同步写入 HDF5。
        Uses Plotter to generate default PNGs and persist them into HDF5.
        """
        if self.hdf5:
            self.callback.on_progress(-1, "生成图表...")
            plotter = Plotter(self.hdf5)
            plotter.save_all(self.png_output_dir)
    ...
    def run_sorting_step(self, step: int) -> None:
        ...
        annealing_path = os.path.join(
            os.path.dirname(os.path.abspath(self.input_file)), annealing_file
        )
    ...
    def archive_files(self, step: int) -> None:
        ...
        self.file_manager.cleanup_old_files(
            step, self.config.layer_mode, self.workdir, self.result_dir
        )

        if step == self.config.generation_steps - 1:
            self.file_manager.cleanup_final_files(
                step, self.config.layer_mode, self.workdir, self.result_dir
            )

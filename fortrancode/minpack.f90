...
!
  ...
  do j = 1, n
    ...
    !   print*,"num_parm",n,"num_angle",m
    !   print*, "X",x(j),x(j)+h
    !   print*, "y+",wa(1:m)
    !   print*, "fvec",fvec(1:m)
    !   print*,"num_fdj",sum(fjac(:,7))
    !   stop
...
! ! +++ 添加以下代码：打印第7个参数的梯度（排序前） +++
! if (n >= 7) then
!   print *, 'Gradient for parameter 7 (before pivoting), iteration:', iter
!   do i = 1, m
!     print *, fjac(i, 7)
!   end do
! else
!   print *, 'n < 7: No parameter 7 gradient to display'
! end if
...
subroutine lmdif1 ( fcn, m, n, x, fvec, tol, maxcall, info )
! maxcall: Maximum number of calling fcn. Added by Tian Lu
!*****************************************************************************80
!
...
!
  ...
  call lmdif ( fcn, m, n, x, fvec, ftol, xtol, gtol, maxcall, epsfcn, &
    diag, mode, factor, nprint, info, nfev, fjac, ldfjac, ipvt, qtf )

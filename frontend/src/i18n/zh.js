export default {
  app: {
    name: 'PolymCrystIndex',
    version: 'v1.9.4'
  },
  home: {
    selectModule: '选择功能模块',
    subtitle: '选择一项功能开始使用',
    title: 'PolymCrystIndex',
    localBadge: '本地工作空间',
    labels: {
      workbench: '工作台',
      workflow: '分析流程',
      preprocess: '预处理',
      results: '结果复核'
    },
    tags: {
      rawImage: '原始图像',
      integration2d: '二维积分'
    },
    recentUpdates: {
      eyebrow: '最近更新',
      expand: '展开摘要',
      collapse: '收起摘要',
      collapsedSummary: '点击展开可查看本次版本更新内容。',
      versionValue: 'v 1.9.4',
      keywords: {
        angleZeroing: '强制直角归正',
        angleZeroingFinal: '结尾强制正交归正',
        uniqueAssignment: '衍射峰唯一指派',
        deduplicate: '衍射峰独占模式（deduplicate）',
        outputMillerFix: 'outputMiller 展示注入清理',
        multiFullMiller: '多组 FullMiller.txt 导入支持',
        manualCell: '手动晶胞参数-新功能',
        glideC: '晶胞参数c轴滑移可视化',
        indexingVisual: '指标化程序可视化',
        utf16Import: 'UTF-16文件导入支持',
        angleWarning: 'psi角度范围预警',
        uiPolish: 'UI布局与交互改进',
        fixedLMode: '固定l约束模式',
        resultFlow: '结果验证流程改进',
        markerSize: 'Miller标记可见性',
        equivalentCell: '等价晶胞探索（向量重置法）',
        psiRangeCheck: '衍射峰角度范围检查',
        peakClickFix: '原始图峰位点击修复'
      },
      dateLabel: '改动日期',
      types: {
        feature: '新增功能',
        fix: '修复改进',
        notice: '使用提醒'
      },
      items: {
        peakClickFix: {
          date: '2026.6.16',
          title: '原始图峰位点击修复',
          summary: '修复原始图像峰提取中「主点击 → 缩放精修 → 积分」链路的坐标对应偏差。此前缩放精修窗口以区内最亮点作为参考重新定位，与生成窗口时的主点击中心不一致，导致精修点击落在偏离真实峰的位置、积分红线指向错误区域。现已统一以主点击点为参考，点击位置与积分结果精确对应。'
        },
        psiRangeCheck: {
          date: '2026.6.5',
          title: '衍射峰角度范围检查',
          summary: '指标化数据导入时自动检查所有衍射峰的 psi 角是否在 -5° ~ 95° 范围内（第一象限，赤道面在 0°）。超出范围的峰会显示醒目警告，但不阻止继续操作。同时优化了输入帮助区域的可见性，角度要求卡片更加突出。'
        },
        equivalentCell: {
          date: '2026.6.5',
          title: '等价晶胞探索（向量重置法）',
          summary: '在手动晶胞参数面板中新增向量重置功能，允许用户将 hk0 指数重新指认（如将 100 视为 110），实时计算等价晶胞的 a*、b*、γ* 及投影长度，辅助判断等价晶胞描述。'
        },
        angleZeroingFinal: {
          date: '2026.5.19',
          title: '结尾强制正交归正',
          summary: '指标化完成后自动判断 α/β/γ 角度是否接近 90°，对符合条件者强制归零并重新评估残差变化。若精度未显著下降，则输出完整 90° 正交晶胞下的 Miller 指数，获得更规整的晶胞参数。'
        },
        uniqueAssignment: {
          date: '2026.5.19',
          title: '衍射峰唯一指派',
          summary: '防止同一衍射峰 (hkl) 被重复匹配到多个观测峰，消除重复指派导致的指标偏倚。每个 hkl 仅保留残差最小的匹配，其余自动降级或剔除，确保指标化结果更加合理可靠。'
        },
        manualCell: {
          date: '2026.4.17',
          title: '手动晶胞参数 - 新功能',
          summary: '支持在主页入口后快速进入手动晶胞参数流程，并围绕当前批量生成结果进行更顺畅的浏览和复核。'
        },
        glideVisual: {
          date: '2026.4.17',
          title: '晶胞参数 c 轴滑移可视化',
          summary: 'GLIDE 结果支持围绕 c 轴滑移变换进行更直观的可视化查看，帮助比对不同分组生成的 FullMiller 标记。'
        },
        indexingVisual: {
          date: '2026.4.17',
          title: '指标化程序可视化',
          summary: '指标化程序中的结果浏览与可视化联动已进一步增强，方便在同一工作流内查看图像、参数与标记变化。'
        },
        fixedLMode: {
          date: '2026.4.26',
          title: '固定l约束模式',
          summary: '支持在遗传算法指标化过程中固定l值约束，提升已知层间距场景下的收敛效率。'
        },
        resultFlow: {
          date: '2026.4.26',
          title: '结果验证流程',
          summary: '指标化与结果页间采用按钮导航，自动预载outputMiller，实现更流畅的结果复核。'
        },
        markerSize: {
          date: '2026.4.26',
          title: 'Miller标记可见性增强',
          summary: '增大原始图和积分图上Miller标记的尺寸和线宽，提升标记辨识度。'
        },
      },
      checkForUpdates: '检查更新',
      checking: '检查中...',
      alreadyLatest: '当前已是最新版本 (v1.9.2)',
      alreadyLatestToast: '已完成检查：当前已是最新版本。',
      statusLabel: '更新状态',
      currentVersion: '当前版本',
      latestVersion: '已发布最新版本',
      localPublishedVersion: '本地发布记录',
      checkedFrom: '检查来源',
      openOfficial: '前往官网',
      openGithub: '查看 GitHub',
      fallbackNotice: '本次检查已自动回退到备用源，结果可能略晚于 GitHub 正式发布。',
      sources: {
        github: 'GitHub',
        gitee: 'Gitee（备用）',
        none: '未获取到远程源'
      },
      statusText: {
        update_available: '发现新版本',
        up_to_date: '当前已是最新发布版',
        ahead_of_release: '当前版本高于已发布版本',
        check_unavailable: '暂时无法完成更新检查'
      },
      summary: {
        updateAvailable: '当前版本为 {current}，已发布最新版本为 {latest}。你可以前往官网或 GitHub 查看详情。',
        latest: '当前运行版本 {current} 已与已发布版本保持一致。',
        aheadOfRelease: '当前运行版本 {current} 高于已发布版本 {latest}，这通常表示你正在使用本地开发版或尚未发布的服务器端代码。',
        unavailable: '未能从远程发布源获取最新版本信息，请稍后重试。'
      }
    },
    cloudDownload: {
      title: '桌面版',
      heading: '桌面版可供下载',
      description: '服务器版提供在线处理能力。如需在本地离线使用，建议先下载桌面版安装包并在 Windows 电脑上运行。',
      downloadButton: '下载桌面版',
      macosNote: 'macOS（未经验证）—— Windows x64 已验证。',
      loading: '加载中...'
    }
  },
  modules: {
    peakExtraction: {
      title: '衍射峰提取',
      desc: '从原始衍射数据中自动识别和提取衍射峰位置'
    },
    indexing: {
      title: '指标化程序',
      desc: '基于遗传算法的晶体结构索引和晶胞参数优化'
    },
    glide: {
      title: 'GLIDE 滑移分析',
      desc: '对晶胞施加滑移剪切变换，批量生成多组 FullMiller 结果',
      batchInput: '批量输入'
    },
    manual: {
      title: '手动晶胞参数',
      desc: '直接输入晶胞参数和波长，批量生成 FullMiller 结果',
      batchInput: '批量输入'
    },
    results: {
      title: '结果展现与处理',
      desc: '可视化分析结果，Miller索引标注',
      visualization: '可视化',
      export: '导出'
    },
    admin: {
      title: '管理员管理',
      desc: '系统配置、用户管理、计算资源监控'
    },
    reserved: '预留',
    active: '可用',
    comingSoon: '即将推出',
    comingSoonDesc: '此功能正在开发中，敬请期待'
  },
  nav: {
    return: '返回主页',
    index: '指标化',
    glide: 'GLIDE',
    manual: '手动',
    dataImport: '数据导入',
    parameters: '参数设置',
    analysis: '运行分析',
    results: '结果导出',
    visualizer: '可视化',
    backToIndexing: '返回指标化'
  },
  status: {
    idle: '就绪',
    running: '分析运行中...',
    completed: '分析完成',
    failed: '失败',
    cancelled: '已取消',
    error: '分析错误'
  },
  login: {
    welcomeBack: '欢迎回来',
    createAccount: '创建账户',
    signInToContinue: '登录以继续使用 PolymCrystIndex',
    registerToStart: '注册以开始您的分析',
    username: '用户名',
    password: '密码',
    school: '学校',
    organization: '单位',
    schoolPlaceholder: '请输入学校名称',
    organizationPlaceholder: '请输入学院、课题组或单位',
    sourceHint: '学校和单位二选一填写即可。',
    sourceRequired: '学校和单位至少填写一项',
    signIn: '登录',
    createAccountBtn: '创建账户',
    alreadyHaveAccount: '已有账户？',
    dontHaveAccount: '没有账户？',
    signInInstead: '登录',
    registerInstead: '注册',
    demoHint: '演示账号：',
    invalidCredentials: '用户名或密码错误',
    registrationSuccess: '注册成功！请登录。'
  },
  brand: {
    tagline: '纤维衍射图谱指标化平台',
    features: {
      ga: '遗传算法优化',
      realtime: '实时分析控制台',
      visualization: '2D/3D 衍射可视化'
    },
    citation: '引用：',
    citationText: 'Ma, T., Hu, W., Wang, D. & Liu, G. (2025). J. Appl. Cryst. 58, 759-767.'
  },
  dataImport: {
    title: '数据导入',
    subtitle: '上传您的衍射数据文件以进行分析',
    dropHere: '将文件拖放到此处',
    orBrowse: '或',
    browse: '浏览',
    format: '.txt',
    formatInfo: 'q, psi, contribution 格式',
    validFile: '文件有效',
    invalidFile: '文件无效',
    dataPoints: '个数据点',
    fileFormat: '文件格式',
    fileFormatDesc: '每行应包含三列，以空白字符分隔：',
    codeExample: 'q值    psi值    贡献度值',
    qLabel: 'q',
    qDesc: '散射矢量大小',
    psiLabel: 'psi',
    psiDesc: '方位角（度）',
    intensityLabel: '贡献度',
    intensityDesc: '数据点的权重贡献',
    helpTitle: '输入帮助',
    helpSubtitle: '上传前请先确认文件格式、角度约定与预处理要求。',
    helpShow: '展开说明',
    helpHide: '收起说明',
    angleRequirements: '角度要求',
    angleRequirementsDesc: '请按纤维衍射指标化所需的角度约定整理数据：',
    angle1: '⚠ 所有衍射峰的 psi 角必须落在 -5° 到 95° 范围内，超出范围的峰将影响指标化结果。',
    angle2: '请使用第一象限数据，并确保 0° 对应赤道面。',
    angle3: '整份文件中的角度正负号与参考方向必须保持一致。',
    psiRangeWarning: '⚠ psi 角度范围警告',
    psiRangeWarningHint: '部分衍射峰的 psi 角超出 -5° ~ 95° 范围。数据应位于第一象限且赤道面在 0°。超出范围的峰可能影响指标化质量，建议检查预处理步骤。',
    psiRangeValid: '✓ 所有 psi 角均在 -5° ~ 95° 范围内',
    preprocessing: '数据预处理',
    preprocessingDesc: '请确保您的数据已经：',
    pre1: '扣除背景',
    pre2: '归一化束流强度',
    pre3: 'psi 角相对于赤道面 (psi=0)',
    continue: '继续设置参数',
    fileEmpty: '文件为空',
    formatError: '每行必须包含至少3列 (q, psi, 贡献度)',
    numericError: '包含非数字字符',
    dataPreview: '数据预览'
  },
  params: {
    title: '参数设置',
    parameters: '仪器参数',
    subtitle: '配置遗传算法和晶胞参数约束',
    gaParams: '遗传算法参数',
    steps: '步数',
    stepsHint: '优化步数',
    stepsTip: '控制每个候选解的局部优化深度。数值越大通常拟合更充分，但单次评估也会更慢。',
    generations: '迭代代数',
    generationsHint: '最大迭代次数',
    generationsTip: '控制遗传算法的总搜索时长。若搜索空间较大或收敛不稳定，可适当增大。',
    populationRatios: '种群比率设置 (%)',
    populationRatiosTip: '存活、交换、变异、新生四项之和必须为 100%。较高的变异/新生更利于探索，存活/交换则更偏向保留优良个体。',
    live: '存活',
    exchange: '交换',
    mutate: '变异',
    newPop: '新生',
    ratioWarning: '比率总和应为 100%（当前 {sum}%）',
    hklExploration: 'HKL 探索范围',
    hklTip: '默认范围适合大多数纤维数据且速度最快。只有在需要更宽的 Miller 指数搜索范围时再使用全面探索或自定义。',
    hklDefault: '默认 (5 5 0)',
    hklFull: '全面探索 (0 0 0)',
    hklCustom: '自定义',
    maxH: '最大 H',
    maxK: '最大 K',
    maxL: '最大 L',
    cellConstraints: '晶胞参数约束',
    cellConstraintsTip: '请给出尽量真实的长度与角度上下限，以缩小搜索空间，同时避免排除合理晶胞。',
    axisMinMax: '{axis} 轴 (Å)',
    angleMinMax: '{angle} 角 (°)',
    advanced: '高级选项',
    wavelength: '波长 (Å)',
    highSymmetry: '高对称因子',
    ompThreads: 'OpenMP 线程数',
    weightFactors: '权重因子',
    azimuth: '方位角 (E1)',
    qValue: 'Q值 (E2)',
    volume: '体积 (E3)',
    glideTitle: 'Glide 剪切批次',
    glideHint: '添加一组或多组 glide 参数。每组都会对优化后的晶胞施加剪切变换，并生成各自的 FullMiller.txt。l0 不能为 0。',
    glideShearTip: 'Glide-shear 在保存基础参数后于专门的 Glide 面板中配置。每个有效分组都要求 l0 非零，并会生成独立批次输出。',
    glidePlacementHint: '该项稍后在专门的 Glide 面板中配置，当前建议先稳定基础约束。',
    glideLabelPlaceholder: '例如 glide_01',
    addGlideGroup: '新增 glide 组',
    fixedPeakTitle: '固定峰约束',
    fixedPeakTip: '只有在你对观测峰序号及其 HKL 指派都很确定时，才建议启用固定峰约束。',
    fixedPeakToggle: '启用固定峰模式',
    fixedPeakFormatLabel: 'peak_index h k l',
    fixedPeakPlaceholder: '1 1 1 0\n3 2 0 1',
    fixedPeakHint: '每行一个峰。格式为 peak_index h k l，其中 peak_index 对应观测衍射峰的序号。',
    angleZeroingToggle: '启用强制直角归正',
    angleZeroingTip: '指标化完成后，检查 α/β/γ 角度是否接近 90°（偏差 < 阈值 T）。对接近 90° 的角度强制设为 90° 后重新计算残差。如果残差上升不超过容差 ΔR_max，则接受该版本，否则保留原始角度。可在不降低指标化质量的前提下获得更规整的直角晶胞参数。',
    angleZeroingThreshold: '角度阈值 (°)',
    angleZeroingTolerance: '残差容忍上限',
    angleZeroingRefineToggle: '启用固定角度重优化',
    angleZeroingRefineTip: '强制直角归正通过后，将归零的角度固定在90°，重新调用Fortran优化器优化 a, b, c 等参数。仅在强制直角归正开启时有效。首次查看结果时需运行 Fortran 优化，可能需要数秒至数十秒。',
    angleZeroingRefineWarning: '⚠ 启用后，首次查看结果导出页面时将运行 Fortran 优化器，可能需要数秒等待。',
    angleZeroingConfirm: '强制直角归正将在指标化完成后，对接近 90° 的 α/β/γ 角度强制设为 90°，并重新计算残差。仅当残差上升不超过容忍上限时才接受该版本。\n\n此操作为后处理评估，不会修改主优化路径。\n\n确定启用？',
    angleZeroingNotice: '⚠ 强制直角归正已启用：指标化完成后将自动检查并归零接近 90° 的角度，重新评估残差。',
    deduplicateToggle: '启用衍射峰唯一指派',
    deduplicateTip: '开启后，每个衍射峰（hkl）最多只能匹配一个观测峰。若多个峰竞争同一 hkl，残差最小者保留，其余自动降级或剔除。可消除重复指派造成的指标偏倚。',
    deduplicatePenaltyLabel: '唯一指派惩罚系数',
    deduplicateTitle: '衍射峰唯一指派结果',
    deduplicateEnabled: '已启用',
    usedHklCount: '已分配 HKL 数',
    totalPeaks: '总衍射峰数',
    conflictsResolved: '冲突解决数',
    deduplicatedPeakIndices: '冲突移除峰索引',
    symmetryApplied: '对称性等效',
    fixedPeakSummaryReady: '已准备 {count} 个固定峰。',
    fixedPeakSummaryEmpty: '未提供固定峰文本。留空将禁用 fixhkl.txt。',
    fixedLTitle: '固定层号 (l)',
    fixedLTip: '启用后，指定峰的层号 l 将被固定为用户输入值，而 h 和 k 由搜索算法自由确定。此模式与固定峰(HKL)模式互斥。',
    fixedLToggle: '启用固定层号模式',
    fixedLFormatLabel: 'peak_index l',
    fixedLPlaceholder: '1 0\n3 1',
    fixedLHint: '每行一个峰。格式为 peak_index l，其中 peak_index 对应观测衍射峰的序号，l 为指定的层号。',
    fixedLSummaryReady: '已准备 {count} 个固定层号。',
    fixedLSummaryEmpty: '未提供固定层号文本。',
    glideSummaryEmpty: '尚未配置 glide 组。分析将不会生成 glide-shear 批次。',
    glideSummaryReady: '已配置 {count} 个 glide 组。',
    glideSummaryInvalid: '警告：有 {count} 个分组的 l0 = 0（无效）。',
    label: '标签',
    remove: '移除',
    lmOptimization: 'LM 优化',
    pseudoOrth: '伪正交约束',
    tiltOpt: '倾斜优化',
    specialFunctions: '特殊功能',
    resetDefaults: '恢复默认',
    saveParams: '保存参数',
    saved: '参数保存成功！'
  },
  console: {
    title: '分析控制台',
    subtitle: '运行遗传算法优化进行晶体结构索引',
    outputLog: '输出日志',
    waiting: '等待分析启动...',
    uploadFirst: '请先上传衍射数据',
    dataReady: '数据文件已就绪：{filename}',
    startAnalysis: '开始分析',
    running: '运行中...',
    cancel: '取消',
    generation: '第 {current} 步 / 共 {total} 步',
    currentError: '当前误差：{value}',
    bestFitness: '最佳适应度：{value}',
    complete: '完成',
    analysisComplete: '分析完成！',
    viewResults: '查看结果',
    progress: '进度',
    status: '状态',
    exportStep: '导出当前步骤',
    exportStepHint: '导出中间结果可视化',
    summary: '摘要',
    detailed: '详细',
    logMode: '日志模式',
    effectiveThreads: '执行线程数',
    adminLimit: '管理员上限',
    requestedThreads: '申请线程数'
  },
  results: {
    title: '结果导出',
    subtitle: '查看和导出分析结果',
    noResults: '暂无结果',
    noResultsDesc: '运行分析后将在此显示结果',
    startAnalysis: '开始分析',
    analysisComplete: '分析完成 - 最佳结构已找到',
    goToPreview: '直达结果预览',
    unitCellParams: '晶胞参数',
    reciprocalCellParams: '倒易空间参数',
    reciprocalCellParamsDesc: '由晶胞参数自动计算',
    equatorialProjection: '赤道面投影',
    aProjection: 'a<sub>投影</sub>',
    bProjection: 'b<sub>投影</sub>',
    millerIndices: 'Miller 索引',
    totalReflections: '总反射数',
    indexedPeaks: '索引峰数',
    structure3D: '3D 晶胞结构',
    structure3DEmpty: '暂无晶胞参数，无法显示 3D 结构',
    rFactor: 'R 因子',
    maxDeviation: '最大偏差',
    deviationPoint: '偏差点 (hkl)',
    deviationValue: '偏差值 (Δq)',
    exportResults: '导出结果',
    downloadPackage: '下载完整结果包',
    packageDesc: '晶胞参数、Miller 索引、输入文件',
    cellOnly: '仅晶胞参数',
    cellDesc: '.txt 格式',
    millerOnly: 'Miller指数下载',
    millerDesc: '所有可能Miller指数',
    exportHDF5: '导出 HDF5 包',
    hdf5Desc: 'HDF5 格式完整流程数据',
    exportVisualization: '导出可视化',
    tiltAngle: '倾斜角',
    qcalc: 'q (计算)',
    psicalc: 'ψ (计算)',
    psiRootCalc: 'ψ (无tilt)',
    colQobs: '观测值 q',
    colQcalc: '计算值 q',
    colQDiff: 'q 差值',
    colPsiObs: '观测值 ψ',
    colPsiWithTilt: 'ψ (有 tilt)',
    colPsiNoTilt: 'ψ (无 tilt)',
    colPsiDiffWithTilt: 'ψ 差值 (有 tilt)',
    colPsiDiffNoTilt: 'ψ 差值 (无 tilt)',
    viewReset: '复位',
    viewA: '沿 A 轴',
    viewB: '沿 B 轴',
    viewC: '沿 C 轴',
    rFactorQ: 'R 因子 (q)',
    rFactorPsi: 'R 因子 (ψ)',
    maxDeviationQ: '最大 Δq',
    maxDeviationQPoint: 'Δq 最大点 (hkl)',
    maxDeviationQPeakNum: 'Δq 最大点编号',
    maxDeviationPsi: '最大 Δψ',
    maxDeviationPsiPoint: 'Δψ 最大点 (hkl)',
    maxDeviationPsiPeakNum: 'Δψ 最大点编号',
    mode: '模式',
    enabled: '已启用',
    disabled: '未启用',
    peaksLabel: '峰 {indices}',
    hkRuleLabel: 'HK 规则',
    passed: '通过',
    failed: '未通过',
    noPeakSymmetryGroups: '当前阈值下没有匹配到 2 峰组或 4 峰组。',
    glideTitle: 'Glide 剪切批次',
    groupsLabel: '分组数',
    batchRootLabel: '批次根目录',
    fullMillerLabel: 'FullMiller',
    outputMillerLabel: 'outputMiller',
    cellLabel: '晶胞',
    goToResultsProcessing: '前往结果处理',
    backToIndexingResults: '返回指标化结果',
    angleZeroingTitle: '强制直角归正',
    angleZeroingAdapted: '✓ 适配',
    angleZeroingNotAdapted: '✗ 不适配',
    angleZeroingStatus: '找寻状态',
    angleZeroingNotEvaluated: '没有角度在阈值 {threshold}° 内接近 90°，跳过强制直角归正',
    refinedCellParams: '固定角度重优化（晶胞参数）',
    zeroedAngles: '归零角度',
    residualChange: '残差变化',
    adaptedCellParams: '适配晶胞参数（归零后）',
    exportAdaptedCell: '导出适配晶胞参数',
    deduplicateTitle: '衍射峰唯一指派结果',
    deduplicateEnabled: '唯一指派已启用',
    usedHklCount: '已分配 HKL 数',
    totalPeaks: '总衍射峰数',
    conflictsResolved: '冲突解决数',
    deduplicatedPeakIndices: '冲突移除峰索引',
    symmetryApplied: '对称性等效'
  },
  visualizer: {
    title: 'Miller 索引可视化',
    subtitle: '将计算的 Miller 索引叠加到衍射图像上',
    dataSource: '数据源',
    rawImage: '原始衍射图',
    integration2D: '2D 积分图',
    loadImage: '加载图像',
    dropImage: '拖放图像或点击上传',
    imageFormats: '.tif / .edf / .cbf',
    poniCalibration: 'PONI 校准',
    importPoni: '导入 .poni 文件',
    poniLoaded: '已从 PONI 加载参数',
    instrumentParams: '仪器参数（无PONI时手动填入）',
    fileImport: '文件导入',
    importDiffractionImage: '导入衍射图像',
    importPoniFile: '导入 PONI 文件（推荐，自动填参）',
    importFullMiller: '导入 FullMiller',
    importOutputMiller: '导入 outputMiller',
    fullMiller: 'FullMiller',
    outputMiller: 'outputMiller',
    points: '点',
    notLoaded: '未加载',
    saveMarkedImage: '保存图像',
    clearFullMiller: '清除 FullMiller',
    clearOutputMiller: '清除 outputMiller',
    clearAllMarkers: '清除全部',
    clearMarkers: '清除标记',
    poniStatus: 'PONI 状态',
    wavelength: '波长',
    pixelX: '像素 X',
    pixelY: '像素 Y',
    centerX: '中心 X',
    centerY: '中心 Y',
    distance: '距离',
    psiRotationOffset: 'ψ 旋转偏移',
    applyParamsAndRedraw: '应用参数 & 重绘',
    contrastAndDisplay: '对比度 & 显示',
    mode: '模式',
    color: '颜色',
    displayControl: '显示控制',
    refreshView: '刷新视图',
    resetZoom: '重置缩放 (fit)',
    showMillerLabels: '显示 Miller 标签',
    quadrant: '象限',
    quadI: '第一象限',
    quadII: '第二象限',
    quadIII: '第三象限',
    quadIV: '第四象限',
    zoom: '缩放',
    fitWindow: '适合窗口',
    zoomIn2x: '放大 2×',
    zoomOut: '缩小',
    loading: '加载中',
    rendering: '渲染中',
    pleaseImportDiffractionImage: '请导入衍射图像',
    import2DIntegrationImage: '导入 2D积分图像',
    importCoordinateInfoFile: '导入坐标信息文件 (processing_info.txt / .json)',
    refreshImage: '刷新图像',
    coordinateRange: '坐标范围',
    azimuthMin: '方位角最小',
    azimuthMax: '方位角最大',
    applyCoordinateRange: '应用坐标范围',
    contrastAndColor: '对比度 & 颜色',
    millerCoordinateMapping: 'Miller 坐标映射 (ψ → Azimuth)',
    psiConvention: 'ψ 约定',
    psiConventionCCW: '图像 Az 与 ψ 反向 (CCW)',
    psiConventionCW: '图像 Az 与 ψ 同向 (CW)',
    psi0CorrespondingAz: 'ψ=0 对应 Az',
    fullMillerCyan: 'FullMiller (青色)',
    outputMillerOrange: 'outputMiller (橙色)',
    azimuthCropDisplay: 'Azimuth 裁切显示',
    enableCrop: '启用裁切',
    from: '从',
    to: '到',
    pleaseImport2DIntegrationImage: '请导入 2D 积分图像',
    cmapGray: '灰度',
    cmapGrayR: '反转灰度',
    cmapHot: '热力图',
    cmapJet: '彩虹',
    resetView: '重置视角',
    rawImageMode: '原始衍射图',
    integrationMode: '2D 积分图',
    loadFailed: '加载图像失败',
    loaded: '已加载',
    importReferencePoints: '导入参考点 (★)',
    importReferencePointsFile: '导入参考点文件',
    referencePoints: '参考点',
    clearReferencePoints: '清除参考点',
    referencePointsCount: '参考点: {count} 个',
    refPointLegend: '★ 参考点',
    refPointImportTip: '支持峰提取导出的 txt/csv 格式',
    boxIntegrateMode: '方框积分',
    boxIntegrateHint: '在图像上拖动绘制矩形，松开后积分并列出方框内的 Miller 指数。',
    boxPanHint: 'Shift + 拖动平移；滚轮缩放。',
    recomputeBox: '重新计算',
    clearBox: '清除方框',
    boxInfo: '方框：{w}×{h} 像素',
    millerInBox: '个方框内 Miller 点',
    boxProfile: '方框积分曲线',
    boxMillerList: '方框内 Miller 指数',
    xAxisQ: 'q (Å⁻¹)',
    xAxis2theta: '2θ (°)',
    xAxisD: 'd (Å)',
    exportProfileCsv: '导出 CSV',
    exportMillerTxt: '导出 TXT',
    noMillerInBox: '方框内无 Miller 指数',
    intensity: '强度',
    source: '来源',
    boxIntegrateFailed: '方框积分失败',
    boxThreshold: '阈值（滤除坏信号）',
    boxThreshMin: '最小',
    boxThreshMax: '最大',
    applyThreshold: '应用',
    resetThreshold: '复位',
    displayUnit: '单位',
    xAxisPx: '像素 (bin)',
    boxCoverage: '方框覆盖',
    hdf5SelectTitle: '选择 HDF5 数据集与切片',
    hdf5SelectPrompt: 'HDF5 文件已加载 — 请选择数据集与切片',
    hdf5NoExtraAxes: '该数据集为 2D — 无额外轴可切分，可直接加载。',
    hdf5Axis: '轴',
    hdf5ModeIndex: '取单帧',
    hdf5ModeMax: '最大值投影',
    hdf5ModeSum: '求和投影',
    hdf5ModeMean: '平均值投影',
    hdf5ProjectionHint: '（沿该轴聚合）',
    hdf5Load: '加载切片',
    hdf5ProbeFailed: 'HDF5 探测失败',
    hdf5LoadFailed: 'HDF5 切片加载失败'
  },
  about: {
    title: '关于',
    subtitle: '了解更多关于 PolymCrystIndex',
    citation: '引用',
    viewPaper: '查看原文',
    acknowledgments: '致谢',
    ack1: '中国科学院化学研究所',
    ack2: '工程塑料实验室',
    ack3: '高分子形态加工课题组',
    ack4: 'HEPS北京怀柔高能同步辐射光源ID08',
    ack5: 'SSRF上海同步辐射光源16B/10U',
    ack6: '致谢 pyFAI 和 fabIO 开发团队提供的方位角积分工具',
    friendlyLinks: '友情链接',
    iccas: '中科院化学所',
    iccasDesc: '中国科学院化学研究所',
    eplab: '工程塑料实验室',
    eplabDesc: '工程塑料实验室',
    pyfaiDesc: '用于X射线衍射的快速方位角积分库',
    pmp: '高分子形态加工课题组',
    pmpDesc: '高分子形态加工课题组',
    polymcrystal: '系列程序首页',
    polymcrystalDesc: '系列程序首页',
    citationSubtitle: '描述核心方法和自动化索引工作流程的同行评审出版物。',
    ackSubtitle: '支持该平台的基础设施和贡献者。',
    linksSubtitle: '连接科学工作流程的相关机构和服务。',
    visit: '访问',
    tutorial: '快速教程',
    researchFocus: '研究重点',
    researchFocusTitle: '纤维衍射指标化',
    researchFocusDesc: '集遗传算法工作流、可视化和可重复科学分析于一体的平台。',
    step1Title: '上传衍射数据',
    step1Desc: '准备纤维衍射数据，格式为 txt，包含 q、psi、贡献度 三列。建议进行背景扣除。',
    step2Title: '配置参数',
    step2Desc: '根据样品信息设置遗传算法参数和晶胞参数约束。',
    step3Title: '运行分析',
    step3Desc: '启动索引过程。算法将使用遗传算法优化晶胞参数。',
    step4Title: '查看和导出结果',
    step4Desc: '查看优化后的晶胞参数、Miller 索引和 3D 晶体结构。导出结果以供进一步分析。'
  },
  serverStatus: {
    title: '服务器状态',
    statusIdle: '空闲',
    statusBusy: '较忙',
    statusFull: '满载',
    currentJobs: '当前计算',
    maxJobs: '最大并行',
    msgIdle: '服务器空闲，算力充足。',
    msgBusy: '服务器较忙，计算可能需要排队。',
    msgFull: '服务器算力已满载，请等待他人计算完成。'
  },
  peakExtraction: {
    rawImage: '原始图像',
    integrated: '积分图像',
    ready: '就绪',
    loading: '加载中...',
    loaded: '已加载',
    error: '错误',
    loadImage: '加载图像',
    dropImage: '拖放图像或点击上传',
    imageFormats: '.edf / .tif',
    loadPoni: '加载 PONI',
    poniNotLoaded: 'PONI 未加载',
    poniLoaded: 'PONI 已加载',
    instrumentParams: '仪器参数',
    wavelength: '波长 (Å)',
    pixelSizeX: '像素 X (μm)',
    pixelSizeY: '像素 Y (μm)',
    centerX: '中心 X (px)',
    centerY: '中心 Y (px)',
    distance: '距离 (mm)',
    applyParams: '应用参数',
    contrast: '对比度',
    contrastMin: '最小值',
    contrastMax: '最大值',
    zoomContrast: '缩放对比度',
    zoomContrastMin: '缩放最小值',
    zoomContrastMax: '缩放最大值',
    applyThreshold: '应用阈值',
    autoThreshold: '自动阈值',
    colormap: '色图',
    gray: '灰度',
    invertedGray: '反转灰度',
    hot: '热力图',
    jet: '彩虹',
    clickToSelect: '点击选择峰',
    zoomClick: '缩放点击',
    integrate: '积分',
    azimuthRange: '方位角范围 (°)',
    radialRange: '径向范围',
    integrateRegion: '积分区域',
    excludeEmptyBins: '排除空 bin(mask 区断开)',
    recordPoint: '记录点',
    recordedPoints: '已记录点',
    exportCsv: '导出 CSV',
    clearRecords: '清除记录',
    qValue: 'q 值',
    psiValue: 'ψ 值 (°)',
    intensity: '强度',
    pixel: '像素',
    refinePeak: '精修峰位',
    importFile: '导入文件',
    importInfo: '导入坐标信息',
    importInfoHint: '坐标信息加载后，需重新点击图象绘制切片曲线',
    setRanges: '设置范围',
    qMin: 'q 最小',
    qMax: 'q 最大',
    azMin: '方位角最小',
    azMax: '方位角最大',
    applyRanges: '应用范围',
    importMiller: '导入 Miller',
    transformMiller: '变换 Miller',
    convention: '约定',
    conventionCCW: '图像 Az 与 ψ 反向 (CCW)',
    conventionCW: '图像 Az 与 ψ 同向 (CW)',
    refAzimuth: '参考方位角 (°)',
    applyTransform: '应用变换',
    getSlice: '获取切片',
    qSlice: 'q 切片',
    azSlice: '方位角切片',
    findPeaks: '寻峰',
    prominence: '峰显著性',
    width: '峰宽度',
    autoFindPeaks: '自动寻峰',
    autoFindPeaksTip: '区域自动寻峰请在较小区域进行，不要框选大量区域',
    qPeaks: 'q 峰',
    azPeaks: '方位角峰',
    recordPeaks: '记录峰',
    deleteRecord: '删除记录',
    peakList: '峰列表',
    showMiller: '显示 Miller',
    loaded: '已加载',
    selectionMode: '选择模式',
    preciseMode: '曲线手动点击',
    roughMode: '区域自动寻峰',
    sliceCenter: '切片中心',
    sliceParams: '切片参数',
    azWidth: '方位角宽度 (°)',
    qDisplay: 'q 显示范围',
    qWidth: 'q 宽度',
    azDisplay: '方位角显示 (°)',
    slicePlots: '一维切片图',
    selectFileCsvTxt: '选择文件 (csv/txt)',
    quickViewCrop: '快速裁切预览',
    cropTo30to120: '基于 Miller 0° 参考，裁切到 −30° 到 +120°',
    integration2DTitle: '二维积分图像',
    peakLabel: '峰',
    applyCrop: '裁切',
    azimuthCrop: '方位角裁切',
    enableCrop: '启用裁切',
    psiConvention: 'ψ 方向约定',
    psiConventionCCW: '图像 Az 与 ψ 反向 (CCW)',
    psiConventionCW: '图像 Az 与 ψ 同向 (CW)',
    psi0CorrespondingAz: 'ψ=0 对应 Az (°)',
    fromRelative: '从（相对0°）',
    toRelative: '到（相对0°）',
    cropOutsideRange: '当前选择超出裁切范围',
    importTxtCsv: '导入 TXT/CSV',
    exportTxt: '导出 TXT',
    exportMarkedImage: '导出标记图片',
    tempRecordTitle: '临时记录说明',
    rawTempRecordHint: '峰提取记录现在只保留在当前会话中，不再写入服务器历史文件。需要复用记录时，请直接导出 txt/csv 或标记图片，再通过上方导入按钮恢复。',
    intTempRecordHint: '积分图峰记录现在只保留在当前会话中，不再写入服务器历史文件。需要复用记录时，请导出 csv 或标记图片，并通过上方导入按钮恢复。',
    psiOffsetLabel: 'ψ offset (°)',
    psiOffsetCorrected: 'corrected = raw − offset'
  },
  admin: {
    overview: '概览',
    users: '用户管理',
    tasks: '任务审计',
    system: '系统状态',
    backToUser: '返回工作台',
    usersDesc: '查看、创建、启用或禁用用户账户',
    school: '学校',
    organization: '单位',
    schoolPlaceholder: '输入学校名称',
    organizationPlaceholder: '输入学院、实验室或单位',
    userSource: '来源',
    approvalStatus: '审核状态',
    approvalStatusHint: '开启后该用户按“已批准用户”默认配额执行；关闭时按普通待审核用户策略执行。',
    approved: '已通过',
    pendingApproval: '待审核',
    quotaSummary: '额度',
    runLimitOverride: '次数覆盖',
    maxThreadsOverride: '线程覆盖',
    runLimitOverrideHint: '仅覆盖该用户的运行次数上限，优先级高于系统默认值；填 0 表示不限次数。',
    maxThreadsOverrideHint: '仅覆盖该用户单次分析可使用的最大线程数；留空则继续使用系统默认线程上限。',
    statusHint: '控制账号是否可登录和提交任务；关闭后该用户将被禁用。',
    defaultUserRunLimit: '普通用户默认次数',
    defaultUserMaxThreads: '普通用户默认线程',
    approvedUserRunLimit: '审核通过默认次数',
    approvedUserMaxThreads: '审核通过默认线程',
    zeroMeansUnlimited: '0 表示无限次数',
    leaveEmptyForDefault: '留空表示使用默认值',
    userRunCountHint: '当前已使用 {count} 次',
    unlimitedRuns: '无限次数',
    usedRuns: '已用 {count} 次',
    maxThreadsShort: '线程',
    tasksDesc: '查看全局任务状态、日志，取消任意运行中任务',
    systemDesc: '监控CPU使用率、活跃任务和服务器健康状态',
    createUser: '创建用户',
    username: '用户名',
    password: '密码',
    displayName: '显示名称',
    role: '角色',
    status: '状态',
    createdAt: '创建时间',
    actions: '操作',
    disable: '禁用',
    enable: '启用',
    disabled: '已禁用',
    noUsers: '未找到用户',
    loadUsersError: '加载用户失败',
    createUserError: '创建用户失败',
    disableUserError: '禁用用户失败',
    enableUserError: '启用用户失败',
    fillRequired: '请填写必填字段',
    usernamePlaceholder: '输入用户名',
    passwordPlaceholder: '输入密码',
    displayNamePlaceholder: '输入显示名称（可选）',
    userId: '用户ID',
    userIdPlaceholder: '按用户ID筛选',
    allStatuses: '全部状态',
    pending: '等待中',
    running: '运行中',
    completed: '已完成',
    failed: '失败',
    cancelled: '已取消',
    progress: '进度',
    bestFitness: '最佳适应度',
    viewLogs: '日志',
    taskLogs: '任务日志',
    noTasks: '未找到任务',
    noLogs: '暂无日志',
    loadTasksError: '加载任务失败',
    loadLogsError: '加载日志失败',
    cancelTaskError: '取消任务失败',
    confirmCancel: '确定要取消此任务吗？',
    cpuUsage: 'CPU使用率',
    activeJobs: '活跃任务',
    serviceStatus: '服务状态',
    systemInfo: '系统信息',
    platform: '平台',
    pythonVersion: 'Python版本',
    version: '版本',
    loadStatusError: '加载系统状态失败',
    refresh: '刷新',
    retry: '重试',
    editUser: '编辑用户',
    resetPassword: '重置密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    passwordMismatch: '两次输入的密码不一致',
    passwordTooShort: '密码长度不能少于6个字符',
    resetPasswordWarning: '重置后用户需使用新密码登录',
    resetPasswordSuccess: '密码重置成功',
    resetPasswordError: '密码重置失败',
    updateUserError: '更新用户信息失败',
    confirmDisable: '确定要禁用用户「{name}」吗？禁用后该用户将无法登录。',
    confirmEnable: '确定要启用用户「{name}」吗？',
    searchPlaceholder: '搜索用户名或显示名称...',
    allRoles: '全部角色',
    allStatuses_filter: '全部状态',
    activeStatus: '已启用',
    disabledStatus: '已禁用',
    noFilteredUsers: '没有匹配的用户',
    save: '保存',
    cancel_action: '取消',
    confirmReset: '确认重置',
    cannotChangeOwnRole: '不能修改自己的角色',
    cannotDisableSelf: '不能禁用自己的账户',
    resetFilters: '重置',
    memoryUsage: '内存使用',
    diskUsage: '磁盘使用',
    autoRefresh: '自动刷新',
    showing: '显示 {start}-{end} 条，共 {total} 条',
    page: '页',
    previousPage: '上一页',
    nextPage: '下一页',
    unknownUser: '未知用户',
    pageSize: '每页',
    totalUsers: '用户总数',
    activeUsers: '活跃用户',
    runningTasks: '运行任务',
    completedTasks: '完成任务',
    systemHealthy: '系统正常',
    systemBusy: '系统繁忙',
    systemFull: '系统满载',
    loadDashboardError: '加载仪表盘失败',
    uptime: '运行时间',
    systemIdle: '空闲',
    systemBusy: '较忙',
    systemFull: '满载',
    totalTasks: '总任务数',
    failedTasks: '失败任务',
    cancelledTasks: '已取消任务',
    cpuPercent: 'CPU使用率',
    startedAt: '开始时间',
    errorMessage: '错误信息',
    cancel: '取消',
    runtimeConfig: '运行配置',
    maxJobs: '最大并发任务数',
    maxOmpThreads: 'OpenMP 最大线程数',
    saveConfig: '保存配置',
    configSaved: '配置已保存',
    configSaveFailed: '保存失败',
    ompLimitHint: '管理员上限：{n}',
    ompRequestedLabel: '申请线程数'
  },
  common: {
    logout: '退出登录',
    admin: '管理员',
    user: '研究员',
    cancel: '取消',
    confirm: '确认',
    close: '关闭',
    loading: '加载中...',
    error: '错误',
    success: '成功',
    warning: '警告',
    info: '提示',
    clear: '清除选择',
    refresh: '刷新',
    retry: '重试',
    active: '已启用',
    localMode: '本地研究模式'
  },
  glide: {
    title: 'GLIDE 滑移分析',
    subtitle: '输入基准晶胞参数和多组滑移剪切参数，批量生成 FullMiller 结果。',
    baseCellParams: '基准晶胞参数',
    sidebarTitle: '简易c滑移',
    reverseSidebarTitle: '反向滑移探索',
    reverseTitle: '反向滑移探索',
    reverseSubtitle: '输入直接晶胞参数，通过多组候选滑移参数探索最可能的滑移方式。',
    reverseIntroTitle: '功能说明',
    reverseIntroDesc: '本功能用于探索最可能的滑移方式。输入直接晶胞参数后，设置多组候选滑移剪切参数（nA、nB、l₀），系统将计算对应的倒易参数 a*、b*、γ*，并给出投影长度 a投影=1/a*、b投影=1/b*，帮助判断哪种滑移方式与观测数据最为吻合。',
    reverseAutoDetectNotice: '自动滑移检测功能将由下一个大版本更新提供。',
    reverseDirectCell: '输入晶胞',
    computedReciprocal: '倒格子参数',
    computedReciprocalHint: '由正格子参数自动计算',
    computedProjection: '投影长度',
    glideShearGroups: '滑移剪切参数',
    label: '标签',
    labelPlaceholder: '例如 glide_01',
    reverseLabelPlaceholder: '例如 reverse_01',
    nA: 'nA',
    nATip: '沿 a 轴的滑移剪切分量（整数，可为负值）',
    nB: 'nB',
    nBTip: '沿 b 轴的滑移剪切分量（整数，可为负值）',
    l0: 'l₀',
    l0Tip: '滑移层数（整数，可为负值，不能为零）',
    removeGroup: '移除该组',
    addGroup: '新增滑移组',
    generate: '生成 GLIDE 结果',
    generating: '生成中...',
    errorNoGroup: '至少需要一组 l₀ 不为零的滑移参数',
    analysisComplete: 'GLIDE 分析完成：共 {count} 组',
    baseCell: '基准晶胞',
    cell: '晶胞',
    volume: '体积',
    reflections: '个反射',
    download: 'Miller指数下载',
    generationFailed: 'GLIDE 生成失败',
    requestFailed: '请求失败',
    viewResult: '查看结果',
    hideResult: '收起结果',
    visualizerTitle: '可视化',
    quickBrowseExpand: '快速浏览',
    quickBrowseCollapse: '收起浏览',
    structure3D: '3D 晶胞结构',
    browseModeSingle: '单组浏览',
    browseModeOverlay: '叠加浏览',
    selectGroup: '选择要浏览的分组',
    overlaySelectionTitle: '选择要叠加的分组',
    overlayHint: '可勾选多个分组进行叠加显示，最多 5 组。',
    liveSyncHint: '导入一次图片/PONI 后，切换分组或重新生成会自动刷新当前标记。',
    selectedCount: '当前已选择 {count} 组',
    overlayLimit: '已达最大叠加数（5 组）',
    reverseCellParams: 'reverse glide 输入参数',
    aStar: 'a*',
    bStar: 'b*',
    gammaStar: 'γ*',
    aProjection: 'a<sub>投影</sub>',
    bProjection: 'b<sub>投影</sub>',
    reverseCandidates: '候选滑移参数',
    l: 'l',
    n: 'n',
    addCandidate: '新增候选',
    removeCandidate: '移除该候选',
    reverseGenerate: '生成反向滑移结果',
    reverseGenerating: '生成中...',
    reverseErrorNoCandidate: '至少需要一组 l₀ 不为零的滑移参数',
    supercellGlideSidebarTitle: '超胞滑移',
    supercellGlideTitle: '超胞滑移分析',
    supercellGlideSubtitle: '先沿 a、b 方向扩展超胞，再对超胞晶胞施加滑移剪切变换。',
    supercellGlideFactors: '超胞倍数（a、b 方向）',
    supercellGlideFactorsTip: '正整数。先将 a 乘以 na、b 乘以 nb 得到超胞，再对超胞施加滑移。',
    supercellGlideShearParams: '滑移剪切参数',
  },
  manual: {
    title: '手动晶胞参数',
    sidebarTitle: '手动晶胞参数',
    subtitle: '输入一组或多组晶胞参数和波长，批量生成 FullMiller.txt 结果。',
    group: '第 {index} 组',
    addGroup: '新增分组',
    generate: '生成 FullMiller（{count} 组{plural}）',
    generating: '生成中...',
    download: 'Miller指数下载',
    generationFailed: '生成失败',
    requestFailed: '请求失败',
    someGroupsFailed: '部分分组失败',
    volume: '体积',
    cell: '晶胞',
    reflections: '个反射',
    viewResult: '查看结果',
    hideResult: '收起结果',
    visualizerTitle: '可视化',
    quickBrowseExpand: '快速浏览',
    quickBrowseCollapse: '收起浏览',
    browseModeSingle: '单组浏览',
    browseModeOverlay: '叠加浏览',
    selectGroup: '选择要浏览的分组',
    overlaySelectionTitle: '选择要叠加的分组',
    overlayHint: '可勾选多个分组进行叠加显示，最多 5 组。',
    liveSyncHint: '导入一次图片/PONI 后，切换分组或重新生成会自动刷新当前标记。',
    selectedCount: '当前已选择 {count} 组',
    overlayLimit: '已达最大叠加数（5 组）',
    reciprocalCell: '倒格子参数',
    reciprocalCellHint: '由正格子参数自动计算',
    projection: '赤道面投影',
    aProjection: 'a<sub>投影</sub>',
    bProjection: 'b<sub>投影</sub>',
    supercellTab: '超胞',
    supercellTitle: '超胞生成',
    supercellSubtitle: '输入晶胞参数和超胞倍数，生成扩展的 FullMiller 结果。',
    supercellFactors: '超胞倍数',
    na: 'na (a轴)',
    nb: 'nb (b轴)',
    nc: 'nc (c轴)',
    supercellFactorTip: '正整数。晶胞参数乘以倍数：a*=a·na, b*=b·nb, c*=c·nc，角度不变。',
    supercellGenerate: '生成超胞 FullMiller（{count}×）',
    supercellInfo: '超胞：{na}×{nb}×{nc} = {total} 个晶胞',
    equivalentCellTitle: '等价晶胞探索（向量重置法）',
    equivalentCellHint: '将旧 hk0 指数重新指认为新 hk0，计算等价晶胞的 a*, b*, γ*。',
    eqOld: '旧',
    eqNew: '新',
    eqConstraint1: '约束 1',
    eqConstraint2: '约束 2',
    eqDefaultHint: '默认第二组为 (0,1)→(0,1)，即保持 b* 方向不变',
    eqArrow: '→',
    computeEquivalent: '计算等价晶胞',
    eqResultTitle: '等价晶胞结果',
    eqErrorSingular: '变换矩阵奇异（行列式为零），请检查输入',
    eqErrorInvalid: '计算结果无效，该变换无物理意义',
    eqAreaRatio: '面积比',
    structure3D: '3D 晶胞结构',
    structure3DHint: '点击展开后展示由生成结果得到的晶胞三维线框结构。'
  }
}
